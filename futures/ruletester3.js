...
const binance = new Binance().options({
  APIKEY: 'RbylZBGeoAysmWD5JT7wd8MqJ2gzmb0YGQM6G',
  APISECRET: 'RbylZBGeoAys1HIPRYAnhDjvyyq6yowgzmb0YGQM6G'
});
...
const doc = new GoogleSpreadsheet('11p27KqmuSl5drcyztssS3SNtA7YGUTE');
...
const webhook = new Discord.WebhookClient('832997472736641105', 'XWlsXwD4NSsuVJJz5JxbgiN0lXcr5jY0tccNRlhErCEGJezNpX');

process.on('unhandledRejection', error => {
  // Will print "unhandledRejection err is not defined"
  console.log('unhandledRejection', error);
  webhook.send('ruletester3\n' + error.message + '\n' + error.name)
});
...
async function tester(symbol, volumeIndex) {
  ...
  for (var t in storageObject) {
    ...
    result['Ratio 1h'] = exits.filter(item => item['profit'] >= 0 && item['time'] >= oneHour.getTime()).length / exits.filter(item => item['time'] >= oneHour.getTime()).length,
      ...
      fs.writeFileSync(`./data/${symbol}PERP_${rule}_transactions.json`, JSON.stringify(transactions, null, 3), {
        encoding: 'utf-8'
      })
