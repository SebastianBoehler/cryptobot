...
const binance = new Binance().options({
  APIKEY: 'RbylZBGeoAysq5be3mWD5JT7wd8MqJ2gzmb0YGQM6G',
  APISECRET: 'RbylZBGeoAys1HIPRiq5be3mWD5JT7wd8MqJ2gzmb0YGQM6G'
});
...
async function main(symbol, mode, index, ruleNo) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    console.log(Math.abs(Indicators15min['eight'] / Indicators15min['twentyone'] - 1), Math.abs(Indicators90min['eight'] / Indicators90min['twentyone'] - 1))
    ...
    let rule = {
      ...
      "Long Exit": [
        [
          Indicators15min['eight'] <= Indicators15min['fiftyfive'] //|| profit >= 0.5 || profit <= -1
        ]
      ],
      "Short Entry": [
        [
    ...
    //console.log('calc', Math.abs(Indicators15min['eight'] - Indicators15min['twentyone']) / price)

    if (index === 3 || ruleNo === 3) {
      //let Indicators60min = await generateIndicators(60, symbol, factor - 2)
      console.log('use alternative rule')
      rule = {
        "Long Entry": [
          [
            //!lastJump || (Number(Data1min[a]['time']) - Number(lastJump)) / 1000 / 60 >= 15,
            Indicators5min['eight'] >= Indicators5min['thirteen'],
            Indicators1min['RSI'] <= 65,
            Indicators5min['RSI'] <= 65,
            Indicators15min['RSI'] <= 60,
            Indicators15min['eight'] >= Indicators15min['fiftyfive'],
            //Indicators60min['eight'] >= Indicators60min['twentyone'],
            //Indicators15min['latest'] >= Indicators15min['volatility'] && 
            Math.abs(Indicators15min['eight'] / Indicators15min['thirteen'] - 1) >= 0.0005
          ]
        ],
        "Long Exit": [
...
async function trigger() {
  ...
  let exchangeInfo = (await binance.futuresExchangeInfo())['symbols']
  exchangeInfo = exchangeInfo.filter(item => item['status'] === 'TRADING' && item['orderTypes'].indexOf('MARKET') >= 0 && item['quoteAsset'] === 'USDT')
  symbolList = exchangeInfo.map(item => {
    return {
      symbol: item['symbol'] + 'PERP',
      mode: item['symbol'] === 'BTCUSDT' ? 'live' : 'DEMO'
    }
  })
  for (var a in symbolList) {
    if (symbolList[a]['symbol'].match(/\d+/) !== null) continue
    //else if (symbolList[a]['symbol'] !== 'BTCUSDTPERP') continue
    let volumeIndex = volumes.findIndex(item => item['symbol'] === symbolList[a]['symbol'])
    if (volumeIndex >= 25) continue
    await main(symbolList[a]['symbol'], symbolList[a]['mode'], symbolList[a]['index'], symbolList[a]['rule'])
    //await sleep(100)
  }
  console.log('\nlist done\n')
  await sleep(150)
