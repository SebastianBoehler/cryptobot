...
const binance = new Binance().options({
  APIKEY: 'RbylZBGeoAys1HIPRYviq5be3mWD5JT7wd8MqJ2gzmb0YGQM6G',
  APISECRET: 'RbylZBGeoAys1HIPRYAnhDjvyWD5JT7wd8MqJ2gzmb0YGQM6G'
});
...
const FTXClient = new RestClient('AYCUnemwv-b9j1LnOhKKDm5cEI', 'ywNqtvLFNt8MgNp8oqr7CRkttfx');
...
async function main() {
  ...
  for (var f in products) {
    ...
    let symbol = products[f]['id'].replace('-', '_')
    if ((symbol.includes('USD') && !symbol.includes('USDC')) || symbol.includes('GBP')) continue
  ...
  for (var a = index; a < tempArray.length; a++) {
    ...
    let volumeIndex = 0 //volumes.findIndex(item => item['symbol'] === symbol + 'PERP')
...
async function tester(symbol, volumeIndex) {
  ...
  let history = await fileUtils.loadData(symbol, 'COINBASE')
    .catch(e => {
      console.error(e)
      return undefined
    })
  ...
  for (var a = 0; a < history.length; a = a + 1) { //9200
    ...
    if (a % 250 == 0 && !endTime) {
      ...
      let temphistory = await fileUtils.loadData(symbol, 'COINBASE')
        .catch(e => {
          console.error(e)
          return undefined
        })
    ...
    let Indicators1min = await generateIndicators(1, symbol, factor - 3, a, undefined, 'COINBASE')
    let Indicators5min = await generateIndicators(5, symbol, factor - 4, a, undefined, 'COINBASE')
    let Indicators15min = await generateIndicators(15, symbol, factor, a, undefined, 'COINBASE')
    //let Indicators60min = await generateIndicators(60, symbol + 'PERP', factor, a - 2)
    let Indicators90min = await generateIndicators(90, symbol, factor - 2, a, undefined, 'COINBASE')
    ...
    for (var u in rules) {
      ...
      //console.log(rule, ruleStorage[rule])
      ...
      //console.log(Indicators90min['twentyone'])

      //console.log(rule, storageObject[rule])
