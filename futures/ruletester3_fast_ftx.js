...
const binance = new Binance().options({
  APIKEY: 'RbylZBGeoAys1HIPRYAWD5JT7wd8MqJ2gzmb0YGQM6G',
  APISECRET: 'RbylZBGeoAys1HI6yow4viq5be3mWD5JT7wd8MqJ2gzmb0YGQM6G'
});
...
const FTXClient = new RestClient('AYCUnemwv-b9j1LKDm5cEI', 'ywNqtvLFNtUcukFCRkttfx');
...
const doc = new GoogleSpreadsheet('11uXOxpfZieOf5p27Kqm3SNtA7YGUTE');
...
let index = 15
if (index === 0) schedule.scheduleJob('00 0 14 * * *', async function () {
...
async function main() {
  let products = await FTXClient.listAllFutures()
  if (products['success']) products = products['result'].filter(item => item['type'] === 'perpetual')
  ...
  for (var f in products) {
    ...
    let symbol = products[f]['name'].replace('-', '')
  ...
  for (var a = index; a < tempArray.length; a++) {
    ...
    let volumeIndex = tempArray.findIndex(item => item === symbol)
    if (volumeIndex >= 20) continue
...
async function tester(symbol, volumeIndex) {
  ...
  let history = await fileUtils.loadData(symbol, 'FTX')
    .catch(e => {
      console.error(e)
      return undefined
    })
  ...
  for (var a = 0; a < history.length; a = a + 1) { //9200
    leverage = 5
    if (!history[a]) {
    ...
    //console.log(endTime && timestamp > endTime, timestamp < customDate.getTime())
    ...
    if (a % 250 == 0 && !endTime) {
      ...
      let temphistory = await fileUtils.loadData(symbol, 'FTX')
        .catch(e => {
          console.error(e)
          return undefined
        })
    ...
    let Indicators1min = await generateIndicators(1, symbol, factor - 3, a, undefined, 'FTX')
    let Indicators5min = await generateIndicators(5, symbol, factor - 4, a, undefined, 'FTX')
    let Indicators15min = await generateIndicators(15, symbol, factor, a, undefined, 'FTX')
    //let Indicators60min = await generateIndicators(60, symbol + 'PERP', factor, a - 2)
    let Indicators90min = await generateIndicators(90, symbol, factor - 2, a, undefined, 'FTX')
    ...
    //console.log(Indicators15min['RSI'])
    ...
    for (var u in rules) {
      ...
      //console.log(Indicators15min['fiftyfive'], Indicators90min['twentyone'])
